...
#include "MANN_NeuralNetwork.h"

MANN_NeuralNetwork::MANN_NeuralNetwork()
{
}
...
void MANN_NeuralNetwork::Predict()
{
	//Normalise Input
	Normalise(X, Xmean, Xstd, Y);

	//Process Gating Network
	for(int i = 0; i < ControlNeurons.Num(); i++)
	{
...
void MANN_NeuralNetwork::StoreParametersDerived()
{
	...
	for(int i = 0; i < YDimBlend; i++)
	{
		FString sIdx = FString::Printf(TEXT("%1d"), i);
		Parameters->Store(Folder + "/cp0_a" + sIdx + ".bin", HDim, XDim, "cp0_a" + sIdx);
		Parameters->Store(Folder + "/cp0_b" + sIdx + ".bin", HDim, 1, "cp0_b" + sIdx);
		Parameters->Store(Folder + "/cp1_a" + sIdx + ".bin", HDim, HDim, "cp1_a" + sIdx);
		Parameters->Store(Folder + "/cp1_b" + sIdx + ".bin", HDim, 1, "cp1_b" + sIdx);
		Parameters->Store(Folder + "/cp2_a" + sIdx + ".bin", YDim, HDim, "cp2_a" + sIdx);
		Parameters->Store(Folder + "/cp2_b" + sIdx + ".bin", YDim, 1, "cp2_b" + sIdx);
	}
}

void MANN_NeuralNetwork::LoadParametersDerived()
{
	...
	CW.SetNum(6 * YDimBlend);
	for(int i = 0; i < YDimBlend; i++)
	{
		FString sIdx = FString::Printf(TEXT("%1d"), i);
		CW[6 * i + 0] = CreateTensor(Parameters->Load("cp0_a" + sIdx));
		CW[6 * i + 1] = CreateTensor(Parameters->Load("cp0_b" + sIdx));
		CW[6 * i + 2] = CreateTensor(Parameters->Load("cp1_a" + sIdx));
		CW[6 * i + 3] = CreateTensor(Parameters->Load("cp1_b" + sIdx));
		CW[6 * i + 4] = CreateTensor(Parameters->Load("cp2_a" + sIdx));
		CW[6 * i + 5] = CreateTensor(Parameters->Load("cp2_b" + sIdx));
	}
